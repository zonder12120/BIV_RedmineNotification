  ...
  try {
    ...
    for (const [id, newIssue] of newIssuesMap) {
      ...
      if (!oldIssue) {
        ...
        !isIgnore(newIssue) &&
        (newIssue.status.name !== oldIssue?.status.name ||
          newIssue.updated_on !== oldIssue?.updated_on)
      ) {
...
// Инициализируем список задач без оповещений, затем с интервалом в 1 мин проходимся функцией main
